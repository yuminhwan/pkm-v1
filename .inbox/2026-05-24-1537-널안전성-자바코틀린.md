---
date: 2026-05-24-1537
type: raw
source-url: "https://kotlinlang.org/docs/java-to-kotlin-nullability-guide.html"
title: "Nullability in Java and Kotlin — Kotlin 공식 마이그레이션 가이드"
tags: [프로그래밍언어, kotlin, java, 널안전성, 언어비교]
---
...
In Java, you can use annotations showing whether a variable can or cannot be `null`. Such annotations aren't part of the standard library, but you can add them separately. For example, you can use JetBrains annotations `@Nullable` and `@NotNull` (from the `org.jetbrains.annotations` package), annotations from JSpecify (`org.jspecify.annotations`), or from Eclipse (`org.eclipse.jdt.annotation`). Kotlin can recognize such annotations when you're calling Java code from Kotlin code and will treat types according to their annotations.
...
interface ArcadeGame<T1> : Game<T1> {
    override fun save(x: T1): T1 // T1 is definitely non-nullable
